...
export default class createReminder extends React.Component {
    ...
    checkDates = async () => {
        var d = new Date()
        this.setState({
            date_1: d.toLocaleDateString().slice(3, 5)
        })
        d.setDate(d.getDate() + 1)
        this.setState({
            date_2: d.toLocaleDateString().slice(3, 5)
        })
        d.setDate(d.getDate() + 1)
        this.setState({
            date_3: d.toLocaleDateString().slice(3, 5)
        })
        d.setDate(d.getDate() + 1)
        this.setState({
            date_4: d.toLocaleDateString().slice(3, 5)
        })
        d.setDate(d.getDate() + 1)
        this.setState({
            date_5: d.toLocaleDateString().slice(3, 5)
        })
        d.setDate(d.getDate() + 1)
        this.setState({
            date_6: d.toLocaleDateString().slice(3, 5)
        })
        ...
        return(
            <View style={{flex: 1, backgroundColor:"#fff"}}>
              <StatusBar backgroundColor={"#fff"} style="dark" />
                <Modal animationInTiming={750} isVisible={this.state.dateModal} backdropOpacity={0.8} useNativeDriver={true}>
                                ...
                                <View style={{flexDirection:"row", justifyContent:"space-between", marginTop:"5%"}}>
                                    <View style={{flexDirection:"row", alignItems:"center"}}>
                                        <Image
                                            ...
                                            style={{height: 16, width: 16}}
                                        />
                                        <Text style={{color:"#444444", fontWeight:"bold", marginLeft: 12}}>Piano</Text>
                                    </View>
                                    {
                                        this.state.ringtone === "Piano" ?
                                            <Entypo name="check" size={14} color="black" style={{padding: 2, height: 18, width: 18, backgroundColor:"#E3EDD2", borderRadius: 100}}/>
                                        :
                                            <></>
                                    }
